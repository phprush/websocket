...
namespace PhpRush\WebSocket\Client;

use PhpRush\WebSocket\Http\Parser;
use PhpRush\WebSocket\Http\WebSocketParser;
use PhpRush\WebSocket\Http\WebSocketFrame;
use PhpRush\WebSocket\Http\WebSocketException;
...
class WebSocket
{
    ...
    protected $haveEncoder = false;
    protected $header;

    const GUID = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11';
    const USERAGENT = 'PhpRush WebSocket Client';
    ...
    public function __construct($host, $port = 80, $path = '/')
    {
        ...
        $this->haveEncoder = method_exists('swoole_websocket_server', 'pack');
        $this->host = $host;
        $this->port = $port;
        $this->path = $path;
     ...
     * @throws WebSocketException
     */
    public function enableCrypto($keyFile = '', $certFile = '')
    {
    ...
    public function connect($timeout = 0.5)
    {
        if (extension_loaded('swoole')) {
            ...
            throw new WebSocketException("Can not find swoole!");
        }
     ...
     * @return bool
     */
    public function doHandShake($headerBuffer)
    {
     ...
     * @return bool
     */
    public function sendJson($data, $masked = true)
    {
